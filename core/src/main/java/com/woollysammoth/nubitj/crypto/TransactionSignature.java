...
package com.woollysammoth.nubitj.crypto;

import com.woollysammoth.nubitj.core.ECKey;
import com.woollysammoth.nubitj.core.Transaction;
import com.woollysammoth.nubitj.core.VerificationException;
...
/**
 * A TransactionSignature wraps an {@link com.woollysammoth.nubitj.core.ECKey.ECDSASignature} and adds methods for handling
...
public class TransactionSignature extends ECKey.ECDSASignature {
     ...
     * and generally violates people's mental model of how Nubit should work, thus, non-canonical signatures are now
     * not relayed by default.
     */
    public static boolean isEncodingCanonical(byte[] signature) {
        // See reference client's IsCanonicalSignature, https://nubittalk.org/index.php?topic=8392.msg127623#msg127623
     ...
     * of the type used by Nubit we have to encode them using DER encoding, which is just a way to pack the two
     * components into a structure, and then we append a byte to the end for the sighash flags.
     */
    public byte[] encodeToNubit() {
        try {
     ...
     */
    public static TransactionSignature decodeFromNubit(byte[] bytes, boolean requireCanonical) throws VerificationException {
        // Nubit encoding is DER signature + sighash byte.
        ...
        // In Nubit, any value of the final byte is valid, but not necessarily canonical. See javadocs for
