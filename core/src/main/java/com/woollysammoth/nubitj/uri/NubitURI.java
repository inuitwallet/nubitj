...
import com.woollysammoth.nubitj.core.Address;
import com.woollysammoth.nubitj.core.AddressFormatException;
import com.woollysammoth.nubitj.core.NetworkParameters;
import com.woollysammoth.nubitj.core.Utils;
...
/**
 * <p>Provides a standard implementation of a Nubit URI with support for the following:</p>
 *
 * <ul>
 ...
 *
 * <ul>
 * <li>{@code nubit:<address>}</li>
 * <li>{@code nubit:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
 ...
 * @see <a href="https://github.com.woollysammoth/bips/blob/master/bip-0021.mediawiki">BIP 0021</a>
 */
public class NubitURI {
    /**
     * Provides logging for this class
     */
    private static final Logger log = LoggerFactory.getLogger(NubitURI.class);
    ...
    public static final String Nubit_SCHEME = "ppcoin";
    ...
    /**
     * Constructs a new NubitURI from the given string. Can be for any network.
     *
     * @param uri The raw URI data to be parsed (see class comments for accepted formats)
     * @throws NubitURIParseException if the URI is not syntactically or semantically valid.
     */
    public NubitURI(String uri) throws NubitURIParseException {
        this(null, uri);
    }

    /**
     * Constructs a new object by trying to parse the input as a valid Nubit URI.
     *
     ...
     *
     * @throws NubitURIParseException If the input fails Nubit URI syntax and semantic checks.
     */
    public NubitURI(@Nullable NetworkParameters params, String input) throws NubitURIParseException {
        checkNotNull(input);
        ...
        try {
            ...
            throw new NubitURIParseException("Bad URI syntax", e);
        }

        // URI is formed as  nubit:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form nubit://address?....
        // We support both until Ben fixes his code.
        
        // Remove the nubit scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : nubit:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        ...
        if (input.startsWith("ppcoin://")) {
            ...
            throw new NubitURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?");
        if (addressSplitTokens.length == 0)
            throw new NubitURIParseException("No data found after the nubit: prefix");
        ...
        if (addressSplitTokens.length == 1) {
            ...
            if (addressSplitTokens.length == 2) {
                ...
                throw new NubitURIParseException("Too many question marks in URI '" + uri + "'");
            }
        }
        ...
        if (!addressToken.isEmpty()) {
            // Attempt to parse the addressToken as a Nubit address for this network
            try {
                Address address = new Address(params, addressToken);
                putWithValidation(FIELD_ADDRESS, address);
            } catch (final AddressFormatException e) {
                throw new NubitURIParseException("Bad address", e);
            }
        }

        if (addressToken.isEmpty() && getPaymentRequestUrl() == null) {
            throw new NubitURIParseException("No address and no r= parameter found");
        }
    }
     ...
     */
    private void parseParameters(@Nullable NetworkParameters params, String addressToken, String[] nameValuePairTokens) throws NubitURIParseException {
        // Attempt to decode the rest of the tokens into a parameter map.
        for (String nameValuePairToken : nameValuePairTokens) {
            final int sepIndex = nameValuePairToken.indexOf('=');
            if (sepIndex == -1)
                throw new NubitURIParseException("Malformed Nubit URI - no separator in '" +
                        nameValuePairToken + "'");
            if (sepIndex == 0)
                throw new NubitURIParseException("Malformed Nubit URI - empty name '" +
                        nameValuePairToken + "'");
     ...
     */
    private void putWithValidation(String key, Object value) throws NubitURIParseException {
        if (parameterMap.containsKey(key)) {
            throw new NubitURIParseException(String.format("'%s' is duplicated, URI is invalid", key));
        } else {
    ...
    /**
     * The Nubit Address from the URI, if one was present. It's possible to have Nubit URI's with no address if a
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("NubitURI[");
        boolean first = true;
    ...
    public static String convertToNubitURI(Address address, BigInteger amount, String label, String message) {
        return convertToNubitURI(address.toString(), amount, label, message);
    }

    /**
     * Simple Nubit URI builder using known good fields.
     * 
     * @param address The Nubit address
     * @param amount The amount in nanocoins (decimal)
     * @param label A label
     * @param message A message
     * @return A String containing the Nubit URI
     */
    public static String convertToNubitURI(String address, @Nullable BigInteger amount, @Nullable String label,
        ...
        builder.append(Nubit_SCHEME).append(":").append(address);
        
        boolean questionMarkHasBeenOutput = false;
        
        if (amount != null) {
            builder.append(QUESTION_MARK_SEPARATOR).append(FIELD_AMOUNT).append("=");
            builder.append(Utils.nubitValueToPlainString(amount));
