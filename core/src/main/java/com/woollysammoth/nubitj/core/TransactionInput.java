...
public class TransactionInput extends ChildMessage implements Serializable {
    ...
    protected void nubitSerializeToStream(OutputStream stream) throws IOException {
        outpoint.nubitSerialize(stream);
     ...
     * "from address" is not well defined in Nubit and you should not assume that senders of a transaction can
     ...
     * in nodes memory pools if the existing version is time locked. See the Contracts page on the Nubit wiki for
