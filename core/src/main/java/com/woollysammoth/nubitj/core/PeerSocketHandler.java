...
import com.woollysammoth.nubitj.net.AbstractTimeoutHandler;
import com.woollysammoth.nubitj.net.MessageWriteTarget;
import com.woollysammoth.nubitj.net.StreamParser;
...
public abstract class PeerSocketHandler extends AbstractTimeoutHandler implements StreamParser {
    ...
    private NubitSerializer.NubitPacketHeader header;
    ...
    public PeerSocketHandler(NetworkParameters params, InetSocketAddress remoteIp) {
        serializer = new NubitSerializer(checkNotNull(params));
    ...
    public int receiveBytes(ByteBuffer buff) {
        checkArgument(buff.position() == 0 &&
                buff.capacity() >= NubitSerializer.NubitPacketHeader.HEADER_LENGTH + 4);
        try {
     ...
     * called automatically by {@link com.woollysammoth.nubitj.net.NioClient} or
     * {@link com.woollysammoth.nubitj.net.NioClientManager} once the socket finishes initialization.
     */
    @Override
