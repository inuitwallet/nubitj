...
import com.woollysammoth.nubitj.core.TransactionConfidence.ConfidenceType;
import com.woollysammoth.nubitj.crypto.KeyCrypter;
import com.woollysammoth.nubitj.crypto.KeyCrypterException;
import com.woollysammoth.nubitj.crypto.KeyCrypterScrypt;
import com.woollysammoth.nubitj.script.Script;
import com.woollysammoth.nubitj.script.ScriptBuilder;
import com.woollysammoth.nubitj.script.ScriptChunk;
import com.woollysammoth.nubitj.store.UnreadableWalletException;
import com.woollysammoth.nubitj.store.WalletProtobufSerializer;
import com.woollysammoth.nubitj.utils.ListenerRegistration;
import com.woollysammoth.nubitj.utils.Threading;
import com.woollysammoth.nubitj.wallet.*;
import com.woollysammoth.nubitj.wallet.WalletTransaction.Pool;
...
import org.nubitj.wallet.Protos.Wallet.EncryptionType;
...
import static com.woollysammoth.nubitj.core.Utils.nubitValueToFriendlyString;
import static com.woollysammoth.nubitj.core.Utils.nubitValueToPlainString;
 ...
 *
 * <p>Wallets can be serialized using either Java serialization - this is not compatible across versions of nubitj,
 ...
 * {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, com.woollysammoth.nubitj.wallet.WalletFiles.Listener)}
...
public class Wallet implements Serializable, BlockChainListener, PeerFilterProvider {
    ...
    public void receivePending(Transaction tx, @Nullable List<Transaction> dependencies, boolean overrideIsRelevant) throws VerificationException {
        ...
        try {
            ...
            if (log.isInfoEnabled()) {
                log.info(String.format("Received a pending transaction %s that spends %s PPC from our own wallet," +
                        " and sends us %s PPC", tx.getHashAsString(), Utils.nubitValueToFriendlyString(valueSentFromMe),
                        Utils.nubitValueToFriendlyString(valueSentToMe)));
            }
    ...
    private void receive(Transaction tx, StoredBlock block, BlockChain.NewBlockType blockType,
        ...
        log.info("Received tx{} for {} PPC: {} [{}] in block {}", sideChain ? " on a side chain" : "",
                nubitValueToFriendlyString(valueDifference), tx.getHashAsString(), relativityOffset,
        ...
        if (!insideReorg && bestChain) {
            ...
            log.info("Balance is now: " + nubitValueToFriendlyString(newBalance));
            if (!wasPending) {
        ...
        for (TransactionInput input : tx.getInputs()) {
            ...
            if (result == TransactionInput.ConnectionResult.ALREADY_SPENT) {
                if (fromChain) {
                    ...
                    log.warn("{}: {}", tx.getHash(), new String(Hex.encode(tx.unsafeNubitSerialize())));
                    Transaction other = input.getConnectedOutput().getSpentBy().getParentTransaction();
                    log.warn("{}: {}", other.getHash(), new String(Hex.encode(tx.unsafeNubitSerialize())));
                }
    ...
    public static class SendResult {
        /** The Nubit transaction message that moves the money. */
    ...
    public static class SendRequest {
         ...
         * (its value is ignored and set to {@link com.woollysammoth.nubitj.core.Wallet#getBalance()} - the fees required
         ...
         * is how mining is incentivized in later years of the Nubit system when inflation drops. It also provides
         ...
         *
         * <p>Once {@link Wallet#completeTx(com.woollysammoth.nubitj.core.Wallet.SendRequest)} is called, this is set to the
         ...
         * 
         * <p>Nubit: This is zero even though we add a required fee. This is an extra fee on top that we don't want.</p>
         */
        ...
        /**
         * If not null, the {@link com.woollysammoth.nubitj.wallet.CoinSelector} to use instead of the wallets default. Coin selectors are
     ...
     *
     * @param address       The Nubit address to send the money to.
    ...
    public void completeTx(SendRequest req) throws InsufficientMoneyException {
        ...
        try {
            ...
            if (bestChangeOutput != null) {
                ...
                log.info("  with {} coins change", nubitValueToFriendlyString(bestChangeOutput.getValue()));
            }
            final BigInteger calculatedFee = totalInput.subtract(totalOutput);
            if (calculatedFee.compareTo(BigInteger.ZERO) > 0) {
                log.info("  with a fee of {}", nubitValueToFriendlyString(calculatedFee));
            }
            ...
            // Check size.
            int size = req.tx.nubitSerialize().length;
    ...
    private boolean adjustOutputDownwardsForFee(Transaction tx, CoinSelection coinSelection, BigInteger baseFee, BigInteger feePerKb) {
        ...
        int size = tx.nubitSerialize().length;
     ...
     * If {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, com.woollysammoth.nubitj.wallet.WalletFiles.Listener)}
    ...
    public String toString(boolean includePrivateKeys, boolean includeTransactions, boolean includeExtensions,
        ...
        try {
            ...
            builder.append(String.format("Wallet containing %s PPC (available: %s PPC) in:%n",
                    nubitValueToPlainString(estimatedBalance), nubitValueToPlainString(availableBalance)));
    ...
    private void toStringHelper(StringBuilder builder, Map<Sha256Hash, Transaction> transactionMap,
        ...
        for (Transaction tx : txns) {
            try {
                ...
                builder.append(Utils.nubitValueToFriendlyString(tx.getValueSentFromMe(this)));
                builder.append(" and receives ");
                builder.append(Utils.nubitValueToFriendlyString(tx.getValueSentToMe(this)));
                builder.append(", total value ");
                builder.append(Utils.nubitValueToFriendlyString(tx.getValue(this)));
                builder.append(".\n");
            } catch (ScriptException e) {
    ...
    public void reorganize(StoredBlock splitPoint, List<StoredBlock> oldBlocks, List<StoredBlock> newBlocks) throws VerificationException {
        ...
        try {
            ...
            log.info("post-reorg balance is {}", Utils.nubitValueToFriendlyString(balance));
     ...
     * of {@link com.woollysammoth.nubitj.core.ECKey#getCreationTimeSeconds()}. This can return zero if at least one key does
    ...
    /**
     * Convenience wrapper around {@link Wallet#encrypt(com.woollysammoth.nubitj.crypto.KeyCrypter,
     ...
     * {@link com.woollysammoth.nubitj.crypto.KeyCrypterScrypt}.
     *
        ...
        try {
            ...
            for (ECKey key : keychain) {
                if (key.isEncrypted()) {
                    ...
                    // (all nubit controlled by that private key is lost forever).
    ...
    /**
     * <p>Convenience wrapper around {@link Wallet#addNewEncryptedKey(com.woollysammoth.nubitj.crypto.KeyCrypter,
            ...
            for (int i = 0; i < tx.getOutputs().size(); i++) {
                ...
                try {
                    if (isTxOutputBloomFilterable(out)) {
                        ...
                        filter.insert(outPoint.nubitSerialize());
                    }
                } catch (ScriptException e) {
                    throw new RuntimeException(e); // If it is ours, we parsed the script correctly, so this shouldn't happen
                }
     ...
     * you can use {@link com.woollysammoth.nubitj.utils.Threading#waitForUserCode()} to block until the future had a
        ...
        public FeeCalculation(SendRequest req, BigInteger value, List<TransactionInput> originalInputs,
            ...
            while (true) {
                ...
                if (change.compareTo(BigInteger.ZERO) > 0) {
                    ...
                    size += changeOutput.nubitSerialize().length + VarInt.sizeOf(req.tx.getOutputs().size()) - VarInt.sizeOf(req.tx.getOutputs().size() - 1);
                ...
                size += req.tx.nubitSerialize().length;
            ...
            if (selection2 == null && selection1 == null) {
                ...
                log.warn("Insufficient value in wallet for send: needed {} more", nubitValueToFriendlyString(valueMissing));
     ...
     * sending transactions to the Nubit network by default. Some sendCoins methods let you specify a broadcaster
     ...
     */
    public void setTransactionBroadcaster(@Nullable com.woollysammoth.nubitj.core.TransactionBroadcaster broadcaster) {
    ...
    private Transaction rekeyOneBatch(long keyRotationTimestamp, final TransactionBroadcaster broadcaster) {
        ...
        try {
            ...
            checkState(rekeyTx.nubitSerialize().length < Transaction.MAX_STANDARD_TX_SIZE);
            commitTx(rekeyTx);
     ...
     * {@link com.woollysammoth.nubitj.core.PeerFilterProvider} interface and generally should not be used directly by apps.
