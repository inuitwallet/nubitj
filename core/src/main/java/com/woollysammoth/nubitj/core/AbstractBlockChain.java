 ...
 * the lightweight SPV mode, a {@link com.woollysammoth.nubitj.store.SPVBlockStore} is the right choice.</p>
 *
 ...
 * {@link com.woollysammoth.nubitj.store.H2FullPrunedBlockStore} implements full verification, which is equivalent to the
...
public abstract class AbstractBlockChain {
     ...
     *
     * Following this one down to the genesis block produces the story of the economy from the creation of Nubit
        ...
        for (Transaction tx : transactions) {
            try {
                if (listener.isTransactionRelevant(tx)) {
                    ...
                    if (clone)
                        tx = new Transaction(tx.params, tx.nubitSerialize());
    ...
    private void checkTestnetDifficulty(StoredBlock storedPrev, Block prev, Block next) throws VerificationException, BlockStoreException {
        ...
        // There is an integer underflow bug in nubit-qt that means mindiff blocks are accepted when time
        // goes backwards.
