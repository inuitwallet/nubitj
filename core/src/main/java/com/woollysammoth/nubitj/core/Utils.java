...
/**
 * A collection of various utility methods that are helpful for working with the Nubit protocol.
 * To enable debug logging from the library, run with -Dnubitj.logging=true on your command line.
 */
public class Utils {
    ...
    /** The string that prefixes all text messages signed using Nubit keys. */
    public static final String Nubit_SIGNED_MESSAGE_HEADER = "Nubit Signed Message:\n";
    public static final byte[] Nubit_SIGNED_MESSAGE_HEADER_BYTES = Nubit_SIGNED_MESSAGE_HEADER.getBytes(Charsets.UTF_8);

    // TODO: Replace this nanocoins business with something better.

    /**
     * How many "nanocoins" there are in a Nubit.
     * <p/>
     * A nanocoin is the smallest unit that can be transferred using Nubit.
    ...
    /**
     * How many "nanocoins" there are in 0.01 Nubits.
     * <p/>
     * A nanocoin is the smallest unit that can be transferred using Nubit.
     * The term nanocoin is very misleading, though, because there are only 100 million
     * of them in a coin (whereas one would expect 1 billion).
    ...
    public static BigInteger toNanoCoins(String coins) {
        ...
        if (bigint.compareTo(NetworkParameters.MAX_MONEY) > 0)
            throw new ArithmeticException("Amount larger than the total quantity of Nubits possible specified.");
        return bigint;
    }
     ...
     * standard procedure in Nubit. The resulting hash is in big endian form.
     */
     ...
     */
    public static String nubitValueToFriendlyString(BigInteger value) {
    ...
    /**
     * <p>
     * Returns the given value as a plain string denominated in lat.   
     * The result is unformatted with no trailing zeroes.
     * For instance, an input value of BigInteger.valueOf(150000) nanocoin gives an output string of "0.0015" lat
     * </p>
     * 
     * @param value The value in nanocoins to convert to a string (denominated in lat)
     * @throws IllegalArgumentException
     *            If the input value is null
     */
    public static String nubitValueToPlainString(BigInteger value) {
        if (value == null) {
            throw new IllegalArgumentException("Value cannot be null");
        }
                
        BigDecimal valueInPPC = new BigDecimal(value).divide(new BigDecimal(Utils.COIN));
        return valueInPPC.toPlainString();
    }
     ...
     *
     * <p>Nubit only uses this "compact" format for encoding difficulty targets, which are unsigned 256bit quantities.
     ...
     *
     * <tt><p>[24] "Nubit Signed Message:\n" [message.length as a varint] message</p></tt>
     */
    public static byte[] formatMessageForSigning(String message) {
        try {
            ByteArrayOutputStream bos = new ByteArrayOutputStream();
            bos.write(Nubit_SIGNED_MESSAGE_HEADER_BYTES.length);
            bos.write(Nubit_SIGNED_MESSAGE_HEADER_BYTES);
