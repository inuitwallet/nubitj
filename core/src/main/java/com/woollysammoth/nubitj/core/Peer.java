...
import com.woollysammoth.nubitj.store.BlockStore;
import com.woollysammoth.nubitj.store.BlockStoreException;
...
/**
 * <p>A Peer handles the high level communication with a Nubit node, extending a {@link PeerSocketHandler} which
 * handles low-level message (de)serialization.</p>
 *
 * <p>Note that timeouts are handled by the extended
 * {@link com.woollysammoth.nubitj.net.AbstractTimeoutHandler} and timeout is automatically disabled (using
 * {@link com.woollysammoth.nubitj.net.AbstractTimeoutHandler#setTimeoutEnabled(boolean)}) once the version
...
public class Peer extends PeerSocketHandler {
    ...
    private static class GetDataRequest {
        ...
        // a nasty hack that relies on the fact that nubit-qt is single threaded and processes messages in order.
     ...
     * {@link com.woollysammoth.nubitj.net.NioClientManager#openConnection(java.net.SocketAddress, com.woollysammoth.nubitj.net.StreamParser)}
     * or
     * {@link com.woollysammoth.nubitj.net.NioClient#NioClient(java.net.SocketAddress, com.woollysammoth.nubitj.net.StreamParser, int)}.</p>
     *
    ...
    private void processVersionMessage(VersionMessage m) throws ProtocolException {
        ...
        // nubitj is a client mode implementation. That means there's not much point in us talking to other client
    ...
    public void setDownloadParameters(long secondsSinceEpoch, boolean useFilteredBlocks) {
    	// For nubit we cannot use filtered blocks until the protocol has been upgraded
        lock.lock();
        try {
    ...
    private void blockChainDownloadLocked(Sha256Hash toHash) {
        ...
        //
        //   https://en.nubit.it/wiki/Protocol_specification#getblocks
        //
     ...
     * Once the pong is received the value returned by {@link com.woollysammoth.nubitj.core.Peer#getLastPingTime()} is
     * updated.
    ...
    /**
     * Returns the elapsed time of the last ping/pong cycle. If {@link com.woollysammoth.nubitj.core.Peer#ping()} has never
    ...
    /**
     * Returns a moving average of the last N ping/pong cycles. If {@link com.woollysammoth.nubitj.core.Peer#ping()} has never
