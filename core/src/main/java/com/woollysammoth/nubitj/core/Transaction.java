...
import com.woollysammoth.nubitj.crypto.TransactionSignature;
 ...
 * the minting of new coins. A Transaction object corresponds to the equivalent in the Nubit C++ implementation.</p>
 *
 * <p>Transactions are the fundamental atoms of Nubit and have many powerful features. Read
 ...
 *
 * <p>All Nubit transactions are at risk of being reversed, though the risk is much less than with traditional payment
...
public class Transaction extends ChildMessage implements Serializable {
     ...
     * {@link TransactionOutput#getMinNonDustValue(BigInteger)}. Currently it's 1000 satoshis.
     */
    public static final BigInteger MIN_NONDUST_OUTPUT = BigInteger.valueOf(10000);

    // These are serialized in both nubit and java serialization.
     ...
     * @param msg Nubit protocol formatted byte array containing message content.
    ...
    /**
     * Used by NubitSerializer.  The serializer has to calculate a hash for checksumming so to
     ...
     * value is determined by a formula that all implementations of Nubit share. In 2011 the value of a coinbase
    ...
    public String toString(@Nullable AbstractBlockChain chain) {
        ...
        for (TransactionOutput out : outputs) {
            ...
            try {
                ...
                s.append(" ");
                s.append(nubitValueToFriendlyString(out.getValue()));
                s.append(" PPC");
    ...
    public void clearInputs() {
        ...
        this.length = this.nubitSerialize().length;
    }
     ...
     * and requires external synchronization. Please refer to general documentation on Nubit scripting and contracts
    ...
    /**
     * Same as {@link #addSignedInput(TransactionOutPoint, com.woollysammoth.nubitj.script.Script, ECKey, com.woollysammoth.nubitj.core.Transaction.SigHash, boolean)}
     ...
     * a wrapper around calling {@link Transaction#hashForSignature(int, byte[], com.woollysammoth.nubitj.core.Transaction.SigHash, boolean)}
     ...
     * features of Nubit are developed.</p>
     *
    ...
    public synchronized Sha256Hash hashForSignature(int inputIndex, byte[] connectedScript, byte sigHashType) {
        ...
        //
        //   https://en.nubit.it/wiki/Contracts
        ...
        try {
            ...
            // is a legacy holdover from a previous, broken design of executing scripts that shipped in Nubit 0.1.
            ...
            if ((sigHashType & 0x1f) == (SigHash.NONE.ordinal() + 1)) {
                ...
                if (inputIndex >= this.outputs.size()) {
                    ...
                    // Nubit implementation. The reference client also contains a bug in handling this case:
    ...
    @Override
    protected void nubitSerializeToStream(OutputStream stream) throws IOException {
        uint32ToByteStreamLE(version, stream);
        uint32ToByteStreamLE(time, stream);
        stream.write(new VarInt(inputs.size()).encode());
        for (TransactionInput in : inputs)
            in.nubitSerialize(stream);
        stream.write(new VarInt(outputs.size()).encode());
        for (TransactionOutput out : outputs)
            out.nubitSerialize(stream);
     ...
     * since Nubit 0.8+ a transaction that did not end its lock period (non final) is considered to be non
     ...
     * This is useful in certain types of <a href="http://en.nubit.it/wiki/Contracts">contracts</a>, such as
