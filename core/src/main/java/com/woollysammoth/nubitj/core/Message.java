...
/**
 * <p>A Message is a data structure that can be serialized/deserialized using both the Nubit proprietary serialization
...
public abstract class Message implements Serializable {
     ...
     * @param protocolVersion Nubit protocol version.
    ...
    private void selfCheck(byte[] msg, int offset) {
        if (!(this instanceof VersionMessage)) {
            ...
            byte[] reserialized = nubitSerialize();
    ...
    // These methods handle the serialization/deserialization using the custom Nubit protocol.
    ...
    /**
     * Should only used by NubitSerializer for cached checksum
     *
     * @return the checksum
     */
    byte[] getChecksum() {
    ...
    /**
     * Should only used by NubitSerializer for caching checksum
     *
    ...
    /**
     * Returns a copy of the array returned by {@link Message#unsafeNubitSerialize()}, which is safe to mutate.
    ...
    public byte[] nubitSerialize() {
        byte[] bytes = unsafeNubitSerialize();
    ...
    /**
     * Serialize this message to a byte array that conforms to the nubit wire protocol.
     * <br/>
     ...
     */
    public byte[] unsafeNubitSerialize() {
        ...
        if (parseRetain) {
            ...
            // which means another call to nubitSerialize is coming.  If we didn't recache then internal
    ...
    /**
     * Serialize this message to the provided OutputStream using the nubit wire format.
     *
     * @param stream
     * @throws IOException
     */
    final public void nubitSerialize(OutputStream stream) throws IOException {
    ...
    /**
     * Serializes this message to the provided stream. If you just want the raw bytes use nubitSerialize().
     */
    void nubitSerializeToStream(OutputStream stream) throws IOException {
        log.error("Error: {} class has not implemented nubitSerializeToStream method.  Generating message with no payload", getClass());
    }

    /**
     * This method is a NOP for all classes except Block and Transaction.  It is only declared in Message
     * so NubitSerializer can avoid 2 instanceof checks + a casting.
     */
