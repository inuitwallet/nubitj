...
public class FilteredBlock extends Message {
    ...
    public void nubitSerializeToStream(OutputStream stream) throws IOException {
        if (header.transactions == null)
            header.nubitSerializeToStream(stream);
        else
            header.cloneAsHeader().nubitSerializeToStream(stream);
        merkleTree.nubitSerializeToStream(stream);
    }
