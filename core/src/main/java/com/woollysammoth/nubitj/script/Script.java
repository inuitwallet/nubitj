...
package com.woollysammoth.nubitj.script;
...
import static com.woollysammoth.nubitj.script.ScriptOpCodes.*;
import static com.woollysammoth.nubitj.core.Utils.bytesToHexString;
 ...
 *
 * <p>Nubit transactions don't specify what they do directly. Instead <a href="https://en.nubit.it/wiki/Script">a
...
public class Script {
     ...
     * The concept of a "from address" isn't well defined in Nubit and you should not assume the sender of a
     ...
     * logic is defined by <a href="https://github.com.woollysammoth/bips/blob/master/bip-0016.mediawiki">BIP 16</a>.</p>
     *
     * <p>nubitj does not support creation of P2SH transactions today. The goal of P2SH is to allow short addresses
    ...
    private static void executeCheckSig(Transaction txContainingThis, int index, Script script, LinkedList<byte[]> stack,
        ...
        try {
            TransactionSignature sig  = TransactionSignature.decodeFromNubit(sigBytes, false);
    ...
    private static int executeMultiSig(Transaction txContainingThis, int index, Script script, LinkedList<byte[]> stack,
        ...
        while (sigs.size() > 0) {
            ...
            try {
                TransactionSignature sig = TransactionSignature.decodeFromNubit(sigs.getFirst(), false);
                Sha256Hash hash = txContainingThis.hashForSignature(index, connectedScript, (byte) sig.sighashFlags);
    ...
    public void correctlySpends(Transaction txContainingThis, long scriptSigIndex, Script scriptPubKey,
        ...
        try {
            txContainingThis = new Transaction(txContainingThis.getParams(), txContainingThis.nubitSerialize());
