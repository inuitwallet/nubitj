...
import com.woollysammoth.nubitj.params.UnitTestParams;
import com.woollysammoth.nubitj.store.BlockStore;
import com.woollysammoth.nubitj.store.MemoryBlockStore;
...
import static com.woollysammoth.nubitj.utils.TestUtils.createFakeBlock;
import static com.woollysammoth.nubitj.utils.TestUtils.createFakeTx;
...
public class LazyParseByteCacheTest {
    ...
    public void testBlock(byte[] blockBytes, boolean isChild, boolean lazy, boolean retain) throws Exception {
        ...
        //message structure.
        NubitSerializer bsRef = new NubitSerializer(unitTestParams, false, false);
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        
        NubitSerializer bs = new NubitSerializer(unitTestParams, lazy, retain);
        Block b1;
        Block bRef;
        b1 = (Block) bs.deserialize(ByteBuffer.wrap(blockBytes));
        bRef = (Block) bsRef.deserialize(ByteBuffer.wrap(blockBytes));
        
        //verify our reference NubitSerializer produces matching byte array.
        bos.reset();
    ...
    public void testTransaction(NetworkParameters params, byte[] txBytes, boolean isChild, boolean lazy, boolean retain) throws Exception {
        ...
        //reference serializer to produce comparison serialization output after changes to
        //message structure.
        NubitSerializer bsRef = new NubitSerializer(params, false, false);
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        
        NubitSerializer bs = new NubitSerializer(params, lazy, retain);
        Transaction t1;
        Transaction tRef;
    ...
    private void serDeser(NubitSerializer bs, Message message, byte[] sourceBytes, byte[] containedBytes, byte[] containingBytes) throws Exception {
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
