...
public class NubitSerializerTest {
    ...
    public void testAddr() throws Exception {
        NubitSerializer bs = new NubitSerializer(MainNetParams.get());
        // the actual data from https://en.nubit.it/wiki/Protocol_specification#addr
    ...
    public void testLazyParsing()  throws Exception {
        NubitSerializer bs = new NubitSerializer(MainNetParams.get(), true, false);
    ...
    private void testCachedParsing(boolean lazy)  throws Exception {
        NubitSerializer bs = new NubitSerializer(MainNetParams.get(), lazy, true);
    ...
    @Test
    public void testNubitPacketHeader() {
        try {
            new NubitSerializer.NubitPacketHeader(ByteBuffer.wrap(new byte[]{0}));
            fail();
        ...
        try {
            new NubitSerializer.NubitPacketHeader(ByteBuffer.wrap(wrongMessageLength));
            fail();
    ...
    public void testSeekPastMagicBytes() {
        ...
        try {
            new NubitSerializer(MainNetParams.get()).seekPastMagicBytes(ByteBuffer.wrap(brokenMessage));
            fail();
        } catch (BufferUnderflowException e) {
            // expected
        }
