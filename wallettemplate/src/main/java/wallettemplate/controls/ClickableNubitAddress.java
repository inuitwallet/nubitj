...
//
// javac -cp $HOME/.m2/repository/net/glxn/qrgen/1.3/qrgen-1.3.jar:$HOME/.m2/repository/de/jensd/fontawesomefx/8.0.0/fontawesomefx-8.0.0.jar:../../../target/classes:../../../../core/target/nubitj-0.11-SNAPSHOT.jar -d ../../../target/classes/ -source 1.7 -target 1.7 wallettemplate/controls/ClickableNubitAddress.java


/**
 * A custom control that implements a clickable, copyable Nubit address. Clicking it opens a local wallet app. The
 * address looks like a blue hyperlink. Next to it there are two icons, one that copies to the clipboard and another
 * that shows a QRcode.
 */
public class ClickableNubitAddress extends AnchorPane {
    ...
    public ClickableNubitAddress() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("nubit_address.fxml"));
            loader.setRoot(this);
    ...
    public String uri() {
        return NubitURI.convertToNubitURI(getAddress(), null, Main.APP_NAME, null);
    }
    ...
    protected void showQRCode(MouseEvent event) {
        ...
        final Main.OverlayUI<ClickableNubitAddress> overlay = Main.instance.overlayUI(pane, this);
