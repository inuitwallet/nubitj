...
import com.woollysammoth.nubitj.core.AbstractWalletEventListener;
import com.woollysammoth.nubitj.core.DownloadListener;
...
import wallettemplate.controls.ClickableNubitAddress;

import java.math.BigInteger;
import java.util.Date;

import static wallettemplate.Main.nubit;
...
public class Controller {
    ...
    public ClickableNubitAddress addressControl;
    ...
    public void onNubitSetup() {
        nubit.wallet().addEventListener(new BalanceUpdater());
        addressControl.setAddress(nubit.wallet().getKeys().get(0).toAddress(Main.params).toString());
        refreshBalanceLabel();
    }
    ...
    public void refreshBalanceLabel() {
        final BigInteger amount = nubit.wallet().getBalance(Wallet.BalanceType.ESTIMATED);
        balance.setText(Utils.nubitValueToFriendlyString(amount));
    }
}
