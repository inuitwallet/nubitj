...
import com.woollysammoth.nubitj.params.RegTestParams;
import com.woollysammoth.nubitj.params.TestNet3Params;
import com.woollysammoth.nubitj.protocols.payments.PaymentRequestException;
import com.woollysammoth.nubitj.protocols.payments.PaymentSession;
import com.woollysammoth.nubitj.store.*;
import com.woollysammoth.nubitj.uri.NubitURI;
import com.woollysammoth.nubitj.uri.NubitURIParseException;
...
import org.nubitj.wallet.Protos;
...
/**
 * A command line tool for manipulating wallets and working with Nubit.
 */
public class WalletTool {
    ...
    private static org.nubit.protocols.payments.Protos.PaymentRequest paymentRequest;
    ...
    public static class Condition {
        ...
        public boolean matchNubits(BigInteger comparison) {
            try {
    ...
    private static void sendPaymentRequest(String location, boolean verifyPki) {
        if (location.startsWith("http") || location.startsWith("nubit")) {
            ...
            try {
                paymentRequest = org.nubit.protocols.payments.Protos.PaymentRequest.newBuilder().mergeFrom(stream).build();
            } catch(IOException e) {
    ...
    private static void wait(WaitForEnum waitFor) throws BlockStoreException {
        ...
        switch (waitFor) {
            ...
            case BALANCE:
                ...
                if (condition.matchNubits(wallet.getBalance(Wallet.BalanceType.ESTIMATED))) {
                    latch.countDown();
                    break;
                }
                wallet.addEventListener(new AbstractWalletEventListener() {
                    ...
                    public synchronized void onChange() {
                        ...
                        if (condition.matchNubits(balance)) {
                            System.out.println(Utils.nubitValueToFriendlyString(balance));
                            latch.countDown();
                        }
                    }
                });
    ...
    private static void deleteKey() {
        ...
        if (pubkey != null) {
            ...
            try {
                ...
                System.err.println(addr + " does not parse as a Nubit address of the right network parameters.");
                return;
            }
        }
        if (key == null) {
